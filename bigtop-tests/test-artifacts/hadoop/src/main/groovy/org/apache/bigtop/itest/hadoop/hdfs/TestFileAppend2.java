...
FileSystem fs = FileSystem.get(conf);
    try {
      { // test appending to a file.
        ...
        stm.close();
        System.out.println("Wrote and Closed second part of file." + stm.getPos());
...
FileSystem fs = FileSystem.get(conf);
        System.out.println("SN: test testSimpleAppend2");
	...
	fs.delete(file1, true);
        ...
        stm.write(fileContents, 0, mid);
        stm.close();
        System.out.println("Wrote and Closed first part of file." + stm.getPos());
...
//          out = fs.append(new Path("/non-existing.dat"),
//              EnumSet.of(CreateFlag.APPEND, CreateFlag.NEW_BLOCK), 4096, null);
	  out = fs.append(new Path("/non-existing.dat"),  4096, null);
          fail("Expected to have FileNotFoundException");
        } catch(java.io.FileNotFoundException fnfe) {
          System.out.println("Good: got " + fnfe);
          fnfe.printStackTrace(System.out);
...
//                  EnumSet.of(CreateFlag.APPEND, CreateFlag.NEW_BLOCK), 4096, null)
//              : fs.append(testfile);
	      FSDataOutputStream stm = appendToNewBlock ? fs.append(testfile,4096, null) : fs.append(testfile);
