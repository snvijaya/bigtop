...
//      assertTrue(dfs.mkdir(new Path("/mkdir-" + Time.now()), FsPermission.getDefault()));
      assertTrue(dfs.mkdirs(new Path("/mkdir-" + Time.now())));
...
//            FsPermission.getDefault());
        dfs.mkdirs(new Path("/non-exist/mkdir-" + Time.now()));
      } catch (IOException e) {
        expectedException = e;
      }
...
//      NamenodeProtocols nnrpc = cluster.getNameNodeRpc();
	FileSystem fs = FileSystem.get(conf);
		...
		System.out.println("storePath A:" + pathStr); 
	}
	for (int i=0; i< NON_CANONICAL_PATHS.length-1; i++) {
		String str =  NON_CANONICAL_PATHS[i];
		System.out.println("storePath B:" + NON_CANONICAL_PATHS[i] + " " + str);
	}
//      for (String pathStr : NON_CANONICAL_PATHS) {
	for (int i=0; i< NON_CANONICAL_PATHS.length-1; i++) {
        try {
          String pathStr = NON_CANONICAL_PATHS[i];
//          nnrpc.mkdirs(pathStr, new FsPermission((short)0755), true);
          Path storePath = new Path(pathStr);
          System.out.println("****SN: path is " + (pathStr != null && !pathStr.trim().isEmpty()));
          System.out.println("storePath C:" + pathStr + " " + NON_CANONICAL_PATHS[i]);
	  System.out.flush();
          fs.mkdirs(storePath);
